...
void
dim_dist(hsize_t gdim, int nproc, int rank, hsize_t *ldim, hsize_t *start)
{
    *ldim  = gdim / nproc;
    *start = *ldim * rank;
    if (rank < gdim % nproc) {
...
void
printProgress(double percentage, char *pre = NULL)
{
...
main(int argc, char **argv)
{
    ...
    hid_t dset = H5Dcreate(fd, dataset, H5T_NATIVE_FLOAT, fspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
