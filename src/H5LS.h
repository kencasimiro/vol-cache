...
typedef struct _MMAP {
    ...
    void *tmp_buf;    // temporally buffer, used for parallel read: copy the read buffer, return the
                      // H5Dread_to_cache function, the back ground thread write the data to the SSD.
    hsize_t offset;
} MMAP;
...
typedef struct H5LS_cache_io_class_t {
    ...
    void *(*write_data_to_cache)(void *dset, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
                                 hid_t plist_id, const void *buf);
    void *(*write_data_to_cache2)(void *dset, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
                                  hid_t plist_id, const void *buf);
    herr_t (*flush_data_from_cache)(void *dset);
    herr_t (*read_data_from_cache)(void *dset, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
...
typedef struct H5LS_mmap_class_t {
    ...
    void *(*write_buffer_to_mmap)(hid_t mem_space_id, hid_t mem_type_id, const void *buf, hsize_t size,
                                  MMAP *mmap);
