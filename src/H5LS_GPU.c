 ...
 */
static herr_t
H5Ssel_gather_copy(hid_t space, hid_t tid, const void *buf, void *mbuf, hsize_t offset)
{
    // assert(false);
    ...
    for (int i = 0; i < nseq; i++) {
        ...
        CUDA_RUNTIME_API_CALL(
            cudaMemcpy(&mp[offset + off_contig], &p[off[i]], len[i], cudaMemcpyDeviceToHost));
        // CUDA_RUNTIME_API_CALL( cudaMemcpyAsync(&mp[offset+off_contig], &p[off[i]], len[i],
        // cudaMemcpyDeviceToHost, stream0 ) ); CUDA_RUNTIME_API_CALL(
        // cudaMemPrefetchAsync(&mp[offset+off_contig], len[i], 1, 0) );
        off_contig += len[i];
    }
    return 0;
...
static void *
H5LS_GPU_write_buffer_to_mmap(hid_t mem_space_id, hid_t mem_type_id, const void *buf, hsize_t size, MMAP *mm)
{
...
static herr_t
H5LS_GPU_create_write_mmap(MMAP *mm, hsize_t size)
{
    // mm->buf = malloc(size);
    // int gpu_id = -1;
...
static herr_t
H5LS_GPU_create_read_mmap(MMAP *mm, hsize_t size)
{
    // mm->buf = malloc(size);
...
static herr_t
H5LS_GPU_remove_write_mmap(MMAP *mm, hsize_t size)
{
    // free(mm->buf);
    // mm->buf = NULL;
...
static herr_t
H5LS_GPU_remove_read_mmap(MMAP *mm, hsize_t size)
{
    // free(mm->buf);
    // mm->buf = NULL;
    CUDA_RUNTIME_API_CALL(cudaFree(mm->buf));
...
static herr_t
removeFolderFake(const char *path)
{
    return 0;
};
