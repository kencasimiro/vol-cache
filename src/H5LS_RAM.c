...
static void *
H5LS_RAM_write_buffer_to_mmap(hid_t mem_space_id, hid_t mem_type_id, const void *buf, hsize_t size, MMAP *mm)
{
...
static herr_t
H5LS_RAM_create_write_mmap(MMAP *mm, hsize_t size)
{
    mm->buf = malloc(size);
    return 0;
};
...
static herr_t
H5LS_RAM_create_read_mmap(MMAP *mm, hsize_t size)
{
    mm->buf = malloc(size);
    return 0;
}
...
static herr_t
H5LS_RAM_remove_write_mmap(MMAP *mm, hsize_t size)
{
    free(mm->buf);
    mm->buf = NULL;
    return 0;
...
static herr_t
H5LS_RAM_remove_read_mmap(MMAP *mm, hsize_t size)
{
    free(mm->buf);
    mm->buf = NULL;
    return 0;
