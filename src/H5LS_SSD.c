 ...
 */
static herr_t
H5Ssel_gather_write(hid_t space, hid_t tid, const void *buf, int fd, hsize_t offset)
{
...
static herr_t
H5LS_SSD_create_write_mmap(MMAP *mm, hsize_t size)
{
    char dname[255];
    strcpy(dname, mm->fname);
    mkdirRecursive(dirname(dname),
                   0755); // dirname will change dname in linux. therefore, we make copy first.
    struct stat info;
...
static herr_t
H5LS_SSD_remove_write_mmap(MMAP *mm, hsize_t size)
{
...
static void *
H5LS_SSD_write_buffer_to_mmap(hid_t mem_space_id, hid_t mem_type_id, const void *buf, hsize_t size, MMAP *mm)
{
...
static herr_t
H5LS_SSD_create_read_mmap(MMAP *mm, hsize_t size)
{
    char tmp[255];
    strcpy(tmp, mm->fname);
...
static herr_t
H5LS_SSD_remove_read_mmap(MMAP *mm, hsize_t size)
{
    herr_t ret;
    munmap(mm->buf, size);
    close(mm->fd);
