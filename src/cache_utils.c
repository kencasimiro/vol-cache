...
/*
  ...
  On Theta, the memory mapped buffer currently does not work with H5Dwrite, we instead allocate a buffer
  directly to the memory.
*/

void
parallel_dist(size_t gdim, int nproc, int rank, size_t *ldim, size_t *start)
{
    *ldim  = gdim / nproc;
    *start = *ldim * rank;
    if (rank < gdim % nproc) {
 ...
 */
void
set_hyperslab_from_samples(int *samples, int nsample, hid_t *fspace)
{
...
*/
void
get_samples_from_filespace(hid_t fspace, BATCH *samples, bool *contig)
{
...
*/
herr_t
rmdirRecursive(const char *path)
{
    herr_t ret;
    if (debug_level() > 1)
